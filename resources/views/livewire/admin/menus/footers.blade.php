...
new
#[Title('Footer Menu'), Layout('components.layouts.admin')]
    ...
    <x-list-item :item="$footer" no-separator no-hover>
      ...
      <x-slot:actions>
        ...
        @if ($footer->order < $footers->count())
          <x-button icon="s-chevron-down" wire:click="down({{ $footer->id }})" tooltip-left="{{ __('Down') }}"
            spinner />
          @endif
          ...
          <x-button icon="o-trash" wire:click="deleteFooter({{ $footer->id }})"
            wire:confirm="{{ __('Are you sure to delete this footer?') }}" tooltip-left="{{ __('Delete') }}" spinner
