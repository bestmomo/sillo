...
use App\Models\{ Menu, Submenu, Page, Post, Serie, Category };
        ...
        @foreach($menus as $menu)
            <x-list-item :item="$menu" no-separator no-hover>
                ...
                <x-slot:actions>
                    ...
                    @endif
                    <x-button icon="c-arrow-path-rounded-square" link="{{ route('menus.edit', $menu->id) }}" tooltip-left="{{ __('Edit') }}" class="btn-ghost btn-sm text-blue-500" spinner />
                    <x-button icon="o-trash" wire:click="deleteMenu({{ $menu->id }})" wire:confirm="{{__('Are you sure to delete this menu?')}}" tooltip-left="{{ __('Delete') }}" spinner class="btn-ghost btn-sm text-red-500" />
                </x-slot:actions>
            </x-list-item>
                ...
                <x-slot:content>
                    @foreach($menu->submenus as $submenu)
                        <x-list-item :item="$menu" no-separator no-hover>
                            ...
                            <x-slot:actions>
                                @if($submenu->order > 1)
                                    <x-button icon="s-chevron-up" wire:click="upSub({{ $submenu->id }})" tooltip-left="{{ __('Up') }}" spinner />
                                @endif
                                @if($submenu->order < $menu->submenus->count())
                                    <x-button icon="s-chevron-down" wire:click="downSub({{ $submenu->id }})" tooltip-left="{{ __('Down') }}" spinner />
                                @endif
                                <x-button icon="c-arrow-path-rounded-square" link="{{ route('submenus.edit', $submenu->id) }}" tooltip-left="{{ __('Edit') }}" class="btn-ghost btn-sm text-blue-500" spinner />
                                <x-button icon="o-trash" wire:click="deleteSubmenu({{ $menu->id }}, {{ $submenu->id }})" wire:confirm="{{__('Are you sure to delete this submenu?')}}" tooltip-left="{{ __('Delete') }}" spinner class="btn-ghost btn-sm text-red-500" />
                            </x-slot:actions>
                        </x-list-item>
                    @endforeach
