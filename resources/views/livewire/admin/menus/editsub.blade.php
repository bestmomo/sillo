...
new
#[Title('Edit Submenu'), Layout('components.layouts.admin')]
...
<div>
  <x-card class="" title="{{__('Edit a submenu')}}">
    ...
    <x-form wire:submit="save()">
      ...
      @if($subOption == 1)
      <x-select label="{{__('Post')}}" option-label="title" :options="$posts" placeholder="{{__('Select a post')}}"
        wire:model="subPost" wire:change="$refresh" />
      @elseif($subOption == 2)
      ...
      @elseif($subOption == 3)
      <x-select label="{{__('Serie')}}" option-label="title" :options="$series" placeholder="{{__('Select a serie')}}"
        wire:model="subSerie" wire:change="$refresh" />
      @elseif($subOption == 4)
