    ...
    <form wire:submit="save">
        ...
        <label for="content">
            ...
            <x-textarea class="mt-1" name="" id="" cols="30" rows="7" wire:model='content'
                maxlength="{{ $maxChars }}" placeholder='Saisissez votre texte ici...'></x-textarea>

        </label>
        <div class="text-right italic">
            <small>
							{{-- Partie à améliorer largement...
							- Pas de 's' si n < 2
							- Meilleur calcul du nombre de mots
							→ ∃ déjà une librairie... ?
							--}}
							Caractères: <span x-text="$wire.content.length + ' / ' + {{ $maxChars }}"></span>
							- Mots: <span x-text="$wire.content.split(' ').length -1"></span>
            </small>
        </div>
